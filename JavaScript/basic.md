...
배열은 처음부터 배열로 선언된 것이고 유사배열은 숫자로 인덱싱 되어있고 length프로퍼티를 가지고 있는 객체를 말한다. 
선언하면 자동으로 숫자 인덱스가 부여되는 배열과 달리 유사배열은 key값으로 숫자 이외의 것도 지정할 수 있기 때문에 모든 프로퍼티를 대상으로 순회하는(객체에 사용하는 것을 가정한) for in을 사용할 경우 숫자key 이외의  프로퍼티에서 문제가 일어날 수 있다. 유사배열의 각 요소를 수정하고 싶다면 일반 객체처럼 key 값을 불러내 다른 값을 재할당하면 된다. 유사배열을 배열로 바꾸고 싶다면 간단하게 Array.from 내장함수를 사용하면 된다.
